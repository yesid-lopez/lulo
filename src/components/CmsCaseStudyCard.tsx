...
import type { CmsCaseStudy } from '@/utils/cmsCaseStudies';

/**
 * Optional CMS fields the card renders if present.
...
export type CmsCaseStudyCardProps = CmsCaseStudy &
  CaseStudyExtras & {
    index: number;
    total: number;
    /** Hide the award badge even when CMS provides one (e.g. real-world implementations). */
    hideAward?: boolean;
  };
...
const CmsCaseStudyCard = ({
  ...
  const showAward = Boolean(award) && !hideAward;
  const showAwardEvent = Boolean(awardEvent) && !hideAward;
...
export default CmsCaseStudyCard;
