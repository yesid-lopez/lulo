...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
   ALTER TYPE "public"."enum_case_studies_type" ADD VALUE IF NOT EXISTS 'real-implementation';`)
}

export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  await db.execute(sql`
   UPDATE "case_studies" SET "type" = 'featured-project' WHERE "type" = 'real-implementation';
